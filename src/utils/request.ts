...
import { SERVICE_OPTION } from './option'
...
class CopoteRequest implements RequestConfig {
    ...
    this.request.interceptors.request.use(
      config => {
        ...
        if (config.service) {
          ...
          const apiRegExp: RegExp = new RegExp(oldApi)
  ...
  create(config: AxiosRequestConfigs): RequestInstance {
    ...
    const Instance = axios.create({
      baseURL: import.meta.env.VITE_BASE_URL + this.service,
   ...
   */
  cancelAll(showMsg: boolean = false): void {
